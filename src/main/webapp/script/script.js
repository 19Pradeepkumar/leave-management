...
const holidaySectionBody = document.getElementById('holidays-section-body');
...
document.getElementById('my-leave-status').addEventListener('change', function() {
    const selectedStatus = this.value;
    filterAndRenderLeaves(selectedStatus);
});

function filterAndRenderLeaves(status) {
    let filteredData = cache.myAllLeaves;
    if (status !== 'ALL') {
        filteredData = filteredData.filter(request => request.leaveStatus === status);
    }
    renderMyAllLeaves(filteredData);
}
...
function renderMyAllLeaves(data) {
    ...
    data.forEach(request => {
        ...
        console.log(data , 'wdbchbdsibvudbszx bsd')
...
function setProfile(name) {
    ...
    const nameArray = name != null ? name.split(' ') : '';
...
//leave views in second page second button
...
holidays.addEventListener('click',showHolidays);
...
function showHolidays(){
   console.log('in holidays')
           ...
           if (cache.allHolidays) {
               renderAllHolidays(cache.allHolidays);
           } else {
               loadHolidays(cache.allHolidays);
           }
}

async function  loadHolidays(){
        try {
            const response = await fetch("http://localhost:8092/leave-management/holidays");
            const data = await response.json();
            cache.allHolidays = data;
...
function renderAllHolidays(data) {
        ...
        `;
            holidaySectionBody.appendChild(row);
        });
    }
